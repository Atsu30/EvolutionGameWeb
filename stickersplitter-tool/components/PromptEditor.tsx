          ...
          <div key={variable.key}>
            ...
            {variable.inputType === 'stickerTable' || isStickerListFormat(values[variable.key]) ? (
              ...
              <textarea
                ...
                rows={6}
                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-sm font-medium focus:ring-2 focus:ring-line-400 outline-none resize-y"
              />
            ) : (
              <input
                type="text"
                value={values[variable.key]}
                onChange={(e) => handleChange(variable.key, e.target.value)}
                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-sm font-medium focus:ring-2 focus:ring-line-400 outline-none"
              />
            )}
          </div>
        ))}
      ...
      <button
        ...
        className={`w-full py-4 font-black rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2 uppercase italic tracking-tighter ${
          copied
            ...
            : 'bg-line-600 hover:bg-line-700 text-white shadow-line-100'
        }`}
      >
        {copied ? (
          <>
