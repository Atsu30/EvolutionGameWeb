  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={startAdd}
          className="px-4 py-2 bg-line-600 hover:bg-line-700 text-white text-xs font-black rounded-xl transition-colors shadow-md flex items-center gap-1.5"
        >
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium focus:ring-2 focus:ring-line-400 outline-none"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-sm font-mono font-medium focus:ring-2 focus:ring-line-400 outline-none resize-y"
            />
          </div>
          <div className="flex gap-3">
            <button
              ...
              className="px-6 py-3 bg-line-600 hover:bg-line-700 disabled:bg-slate-200 text-white font-black text-xs rounded-xl transition-colors shadow-md uppercase"
            >
              {editingId ? '更新' : '作成'}
            </button>
            <button
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {template.variables.map(v => (
                    <span key={v.key} className="text-[10px] font-bold text-line-600 bg-line-50 px-2 py-0.5 rounded-md">
                      {`{{${v.key}}}`}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  ...
                  className="text-xs font-bold text-slate-400 hover:text-line-600 transition-colors px-2 py-1"
                >
                  編集
                </button>
                <button
            ...
            <button
              ...
              className={`mt-4 w-full py-3 font-black text-sm rounded-xl transition-all flex items-center justify-center gap-2 ${
                activeTemplateId === template.id
                  ...
                  : 'bg-line-50 text-line-600 hover:bg-line-100'
              }`}
            >
