...
const StickerCard: React.FC<StickerCardProps> = ({ sticker, onToggleSelect, onProcess }) => {
  return (
    <div className={`relative group border-2 rounded-xl overflow-hidden transition-all duration-300 ${sticker.isSelected ? 'border-indigo-500 ring-2 ring-indigo-200' : 'border-slate-200 hover:border-slate-300'}`}>
      <div 
      ...
      <div className="p-3 bg-white flex justify-between items-center">
        <button 
          ...
          className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${sticker.processedUrl ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700 hover:bg-indigo-50 hover:text-indigo-700'}`}
        >
